 ...
 * @author PC01017745
 */
class MessageTypeWrongPassword implements Message {

    private final MessageService messageService;

    public MessageTypeWrongPassword(MessageService messageService) {
        this.messageService = messageService;
    }

    @Override
    public Map<String, String> getMessageFromDatabse(SingInOutSessions singInOutSessions) {
        md.ibanc.rm.entities.Message message = messageService.findMessageByTypeAndLang("WrongPassword", singInOutSessions.getCustomers().getLanguages().getShortName());
